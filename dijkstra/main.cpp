/*
Codice lasciato per convenienza, non mantenuto, né concluso. Si riferisca a
alt.cpp per soluzione corretta
*/
...
template <class T> int find_nodo(vector<T> &nodi, T target) {
...
template <class T> pair<T, int> extract_min(vector<pair<T, int>> &Q) {
...
template <class T>
int dijikstra(vector<T> &nodi, vector<vector<T>> &grafo, T sorgente,
              T destinazione) {
  vector<pair<T, int>> Q;
  vector<pair<T, int>> S;
  for (int i = 0; i < nodi.size(); i++) {
