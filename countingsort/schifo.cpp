...
Coppia<double> **CountingSort(Coppia<double> **A, int n, ofstream &out) {
  ...
  for (int i = n; i > 0; i--) {
    B[C[vett[i]->getX() - min + 1]] = new Coppia<double>(
        double(vett[i]->getX() / 10.0), double(vett[i]->getY() / 10.0));
    C[vett[i]->getX() - min + 1]--;
  }
...
template <class H> void printCoppie(Coppia<H> **vett, int n, ofstream &out) {
  for (int i = 1; i <= n; i++)
    out << *vett[i] << " ";
  out << endl;
}
